// views/settings.ts — 设置（模型配置）（Component 模式）。
// 已裁剪为「个人本地」工具：仅保留服务商 / 模型 / API Key / 代理 / 测试连接；移除登录注册等社区账号模块。
...
export function settings(): void {
  ...
  ctx.appEl().innerHTML = `
    ...
    <p class="muted" style="font-size:.82rem;margin-top:6px;">本工具完全本地运行：API Key 只保存在你本机浏览器，不上传任何服务器。模型调用可直连厂商，也可走本地代理（避免跨域 / 隐藏 Key）。</p>
