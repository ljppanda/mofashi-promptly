...
| 层 | 技术 |
|---|---|
| 前端 | Vite + TypeScript（零运行时框架，原生 ES 模块，已按 `core/` + `views/` 组件化分层）+ Tailwind（CDN） |
...
| F6 | 设置 | 配置自有 LLM Key + 模型选择 + 「测试连接」回显实际模型 + 代理开关 |
...
4. **社区发现 / SEO**：全文搜索增强、标签体系、作者主页完善、sitemap / OG 元信息。
